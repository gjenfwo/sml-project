...
typedef struct {
    /*
     * For this test output_state_changed_cb calls that have incorrect values are considered false_positives
     * and calls with correct values are considered true positives.
     * When we are suppose to call output_state_changed_cb, but it isn't called, it is a false_negative.
     * True negatives are when we are not suppose to call output_state_changed_cb and it is not called.
     * Note that if we are suppose to call output_state_changed_cb and it is called with an incorrect value, it is considered a false positive
     * A value is considered correct if the difference between the expected value and the real value is below a threshold.
     */
...
_read_state_cb(struct sml_object *sml, void *data)
{
    ...
    for (i = 0; i < input_len; i++) {
        ...
        value1 = atof(tokens[i]);
        printf("%d : %f\n",i, value1);
        //value2 = atof(tokens[i+1]);
        //value3 = atof(tokens[i+2]);

        sml_variable_set_value(sml, sml_variable, value1);
        if (i != 0)
            printf(", ");
        if (!sml_variable_get_name(sml, sml_variable, var_name,
            sizeof(var_name)))
            //printf("%s: %f %f %f", var_name, value1, value2, value3);
    }
    printf("}, Outputs {");

    for (i = 0; i < output_len; i++) {
        sml_variable = sml_variables_list_index(sml, output_list, i);
        value1 = atof(tokens[input_len + i]);
        ctx->outputs[i] = value1;
        sml_variable_set_value(sml, sml_variable, value1);
        if (i != 0)
            printf(", ");
        if (!sml_variable_get_name(sml, sml_variable, var_name,
            sizeof(var_name)))
            printf("%s: %f", var_name, value1);
    }
    printf("}, Expected {");
