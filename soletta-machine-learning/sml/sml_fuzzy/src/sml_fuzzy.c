...
/*
 * It seems that this function make new engine and initialize
 */
API_EXPORT struct sml_object * sml_fuzzy_new(void)
{
    struct sml_fuzzy_engine *fuzzy_engine = calloc(1, sizeof(struct sml_fuzzy_engine));
    ...
    fuzzy_engine->observation_controller = sml_observation_controller_new(fuzzy_engine->fuzzy);
    ...
    sol_vector_init(&fuzzy_engine->terms_to_be_removed, sizeof(struct sml_term_to_remove));
