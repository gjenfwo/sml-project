...
API_EXPORT bool
sml_set_read_state_callback(struct sml_object *sml, sml_read_state_cb read_state_cb, void *data)
{
    ON_NULL_RETURN_VAL(sml, false);
    struct sml_engine *engine = (struct sml_engine *)sml;
...
API_EXPORT struct sml_variable *
sml_variables_list_index(struct sml_object *sml, struct sml_variables_list *list, uint16_t index)
{
    struct sml_engine *engine = (struct sml_engine *)sml;
...
API_EXPORT bool sml_variable_set_range(struct sml_object *sml, struct sml_variable *sml_variable, float min, float max)
{
    struct sml_engine *engine = (struct sml_engine *)sml;
    ...
    if (max < min) {
        sml_warning("Max value (%f) is lower than min value (%f). Inverting.", max, min);
